...
class MolkkyMatch {
  ...
  MolkkyMatch({
    ...
    required this.type,
       ...
       currentSetRecord = SetRecord(1, players.isEmpty ? '' : players.first.id, players.map((p) => p.id).toList());
  ...
  void prepareNextSet({bool manualOrder = false}) {
    ...
    currentSetRecord = SetRecord(currentSetIndex, players.isEmpty ? '' : players.first.id, players.map((p) => p.id).toList());
